...
        setIsLoading(true);
        try {
            // This fetch populates the 'trips' state which is passed to the map
...
    const handleDeleteTrip = async (tripId) => {
        // IMPORTANT: Custom modal should be used instead of window.confirm in a real deployment
...
    };
    ...
    // --- Content for unauthenticated user (Hata di gayi hai white box) ---
    if (!user) {
        return (
            <div 
            ...
            >
                {/* अब यह कंटेनर पूरी तरह से पारदर्शी है (Only has margin and centering classes) */}
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-48 py-8 text-center">
                    {/* टेक्स्ट ब्लॉक को पढ़ने योग्य बनाने के लिए एक हल्की सफेद पृष्ठभूमि (bg-white/90) दी गई है */}
...
    }
    ...
    // --- Content for authenticated user (Hata di gayi hai white box) ---
    return (
        <div 
            className="min-h-screen bg-cover bg-center"
            style={{ backgroundImage: `url(${BACKGROUND_IMAGE_URL})` }}
        >
            {/* यह मुख्य सामग्री कंटेनर है। इसमें अब कोई पृष्ठभूमि रंग या धुंधलापन (blur) नहीं है। */}
    ...
                {/* Trip Form Toggle */}
                <button
                    className={`flex items-center px-6 py-3 mb-8 text-white font-semibold rounded-xl shadow-lg transition duration-300 ease-in-out transform hover:scale-[1.02] ${
                        showForm
    ...
                    {showForm ? (
                        <>
                            <Trash2 className="w-5 h-5 mr-2" /> Close Form
                        </>
                    ) : (
                        <>
                            <PlusCircle className="w-5 h-5 mr-2" /> Add New Trip
                        </>
                    )}
                </button>
    ...
                {/* ====== MAP INTEGRATION BLOCK ====== */}
                {/* मैप को भी एक हल्की सफेद पृष्ठभूमि (bg-white/90) में लपेटते हैं ताकि यह हमेशा दिखाई दे */}
    ...
                        No road trips have been planned yet. Click 'Add New Trip' to start your first journey!
                    </p>
                ) : (
    ...
                            <div
                                key={trip._id}
                                className="bg-white rounded-xl shadow-xl p-6 flex flex-col justify-between border-t-4 border-indigo-500 hover:shadow-2xl transition duration-300"
                            >
                                <div>
    ...
                                            {trip.createdBy?.username || "Unknown User"}
                                        </span>
                                    </div>
    ...
                                                {/* CRITICAL FIX: Access the 'name' property of the location object */}
                                                {loc.name} 
                                            </li>
                                        ))}
                                    </ul>
